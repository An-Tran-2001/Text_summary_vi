...
def frequency_solve(text):
    ...
    for sentence in sentenceValue:  # Tính tổng giá trị của tất cả các câu
        # Cộng giá trị của từng câu vào tổng giá trị của tất cả các câu
        sumValues += sentenceValue[sentence]
    # Tính giá trị trung bình của tất cả các câu
    average = int(sumValues / len(sentenceValue))

    summary = ''  # Chuỗi tóm tắt
    for sentence in sentences:  # Tạo tóm tắt
        # Nếu giá trị của câu lớn hơn 1.2 lần giá trị trung bình  của tất cả các câu thì thêm vào tóm tắt với
